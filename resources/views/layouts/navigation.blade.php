        ...
        <div class="flex justify-between h-16">
            ...
            <div class="hidden sm:flex sm:items-center sm:ml-6">
                @foreach(config('app.available_locales') as $locale)
                    <x-nav-link :href="route(\Illuminate\Support\Facades\Route::currentRouteName() , ['locale' => $locale , 'id' => '/{id?}/'])" :active="app()->getLocale() == $locale">
                        {{ strtoupper($locale) }}
                    </x-nav-link>
                @endforeach

                    <x-dropdown align="right"  style="margin: 50px 10px;"  width="298">
                    <x-slot name="trigger">
